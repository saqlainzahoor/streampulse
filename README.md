...
**Supporting elements from other tracks**, because reliable insight depends on good input and sustained participation:
...
| Track 3 — AI-Supported Assessment | Submitted readings are checked against a site's recent history. Inconsistent entries are **flagged with a plain-language reason**, never silently accepted or discarded — the volunteer confirms or corrects. This is deliberately a simple, explainable heuristic rather than a black-box model, in keeping with "support, don't replace, human judgment." |
| Track 5 — Community & Gamification | Reporting streaks, badges, and a leaderboard reward volunteers who return, since trend data is only trustworthy if the same sites get monitored repeatedly. |
| Track 6 — Resilience Informatics | The dashboard's top banner surfaces a short-range early warning (e.g. rain forecast → expected turbidity/E. coli spike at specific downstream sites) so agencies can act pre-emptively. |
